...
void LocomotiveBehavior::run()
{
    ...
    while (1) {
        ...
        if (nbTurn >= NB_TURN) {
            if (reverse) {
                attendre_contact(travel.crbegin()->contact);
                loco.afficherMessage("I passed the contact no. " + QString(std::to_string(travel.crbegin()->contact).c_str()));
            } else {
                attendre_contact(travel.cbegin()->contact);
                loco.afficherMessage("I passed the contact no. " + QString(std::to_string(travel.cbegin()->contact).c_str()));
            }
